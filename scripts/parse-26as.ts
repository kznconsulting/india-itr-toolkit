#!/usr/bin/env bun
// Parse a Form 26AS PDF (or its pdftotext -layout text) into per-deductor TDS
// detail. Debug/standalone entry point - the normal path is
// `bun run extract <client-dir>`, which calls the same parser.
//
// Usage:
//   bun scripts/parse-26as.ts <26AS.pdf|26as.txt> [--pan AAAAA9999A --dob DD/MM/YYYY] [--password PW]

import { readFileSync } from "node:fs";
import { pdfToText } from "./lib/pdftext";
import { parse26as } from "./lib/f26as";
...
if (!file) {
  console.error("usage: bun scripts/parse-26as.ts <26AS.pdf|26as.txt> [--pan PAN --dob DD/MM/YYYY] [--password PW]");
  process.exit(1);
}
